...
function createAdminStub(visibleOverrides: Record<string, unknown> = {}) {
  ...
  const users = [
    { id: "user-in-scope", first_name: "Oficial Uno", email: "uno@demo.test", role_level: 1, status: "Activo", assigned: "Casa Pavas" },
    { id: "user-out-scope", first_name: "Oficial Dos", email: "dos@demo.test", role_level: 1, status: "Activo", assigned: "Otro Puesto" },
  ]
...
describe("/api/supervision-findings", () => {
  ...
  it("returns only findings whose supervision is visible to L2", async () => {
    ...
    expect(body.assignees).toEqual([
      expect.objectContaining({ id: "user-in-scope", label: "Oficial Uno" }),
    ])
